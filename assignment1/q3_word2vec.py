...
def normalizeRows(x):
    ...
    ### YOUR CODE HERE
    dist = np.apply_along_axis(lambda x:np.sqrt(x.T.dot(x)), 1, x).reshape(-1,1)
    x /= dist 
    ### END YOUR CODE
...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    ydiff = yhat.reshape(-1,1)
    ydiff[target] -= 1.0
    gradPred = np.dot(outputVectors.T, ydiff)
    print(predicted.shape)
    print(ydiff.shape)
    grad = np.dot(predicted.reshape(-1,1), ydiff.T)
    '''
    vhat = predicted
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    for k in indices[1:]:
        z1 = sigmoid(outputVectors[k].dot(predicted))
        cost -= np.log(1 - z1)
        gradPred += outputVectors[k] * z1
        grad[k] -= predicted * z1
    ### END YOUR CODE
